...
#Purpose: Analysing 2006 childhod asthma incidence rates in the US by loading two data sets the BRFSS and ACBS
...
# Loading data set
load("Data/FIPS")
ACBS_a <- read.spss("Data/ACBS_2006.SAV", 
                    use.value.labels = F,
                    to.data.frame = T, 
                    trim.factor.names = T, 
                    trim_values = T)

BRFSS_a <- read.xport("Data/CDBRFS06.XPT")
...
write.xlsx(Asthma_sample, "Asthma_result.xlsx", sheetName = "2006_count", showNA=F, append = T)
write.xlsx(Asthma_weighted, "Asthma_result.xlsx", sheetName = "2006_weighted", showNA=F, append = T)
write.xlsx(Asthma_IR, "Asthma_IR.xlsx", sheetName = "2006", showNA=F, append = T)
